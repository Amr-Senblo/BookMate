...
// const errorHandler= require('./errorHandlerMiddelware')
...
exports.forgetPassword = async (req, res, next) => {
  ...
  console.log('👤👤👤👤user forgotted password', user);
  if (!user) {
...
exports.resetPassword = async (req, res, next) => {
  ...
  const user = await User.findOne({
    passwordResetToken: hashToken, // remember that the passwordResetToken that saved in the database is hashed using 'createPasswordResetToken' fuction
    passwordResetExpires: { gt: Date.now() },
  });
  // 2) If token has not expired, and ther is user, set the new passsword
  if (!user) {
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  // User.findByIdAndUpdate     will NOT work as validator only works with create and save
